...
{
	this->setSpriteRotAngle(this->getSpriteRotAngle() +(1000.0f * deltaTime));
	...
	direction.X = (sin(PI/180));
	direction.Y = -(cos(PI / 180));
	...
	cout << "Bullet position - y: " << this->getSpritePos().y << " Bullet position - x: " << this->getSpritePos().x << endl;
